...
async def finish_test(q, context):
    ...
    cur.execute(
        ...
        (q.from_user.id, context.user_data["exam"],
         context.user_data["topic"], score, total,
    ...
    await q.edit_message_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔍 Review", callback_data="review_0")]
        ])
    )
...
async def review(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    a = attempts[idx]
    kb = [[InlineKeyboardButton("Next ▶", callback_data=f"review_{idx+1}")]]
    await q.edit_message_text(
        f"❓ {a['question']}\n\n"
...
async def start_new(update, context):
    await exam_select(update, context)

async def go_score(update, context):
    q = update.callback_query
    await q.answer()
    cur.execute("SELECT score,total,test_date FROM scores WHERE user_id=? ORDER BY id DESC LIMIT 5",
                (q.from_user.id,))
    rows = cur.fetchall()
    msg = "📊 My Scores\n\n" + "\n".join(f"{r[0]}/{r[1]} ({r[2]})" for r in rows)
    await q.edit_message_text(msg or "No scores yet")

async def go_perf(update, context):
    q = update.callback_query
    await q.answer()
    cur.execute("SELECT score,total FROM scores WHERE user_id=? ORDER BY id DESC LIMIT 5",
                (q.from_user.id,))
    rows = cur.fetchall()
    msg = "📈 Performance\n\n" + "\n".join(f"{r[0]}/{r[1]}" for r in rows)
    await q.edit_message_text(msg or "No data")
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(review, "^review_"))
