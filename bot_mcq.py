...
BOT_NAME = "@MyScoreCardBot"   # 👈 PDF corner branding
...
async def answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sel == mcq[8]: context.user_data["score"] += 1
    else: context.user_data["wrong"].append(mcq)
...
async def show_wrong(q, context):
    ...
    mcq = w[i]
    await q.edit_message_text(
        f"❌ *Wrong Question {i+1}/{len(w)}*\n\n{mcq[3]}\n\n"
        f"A. {mcq[4]}\nB. {mcq[5]}\nC. {mcq[6]}\nD. {mcq[7]}\n\n"
        f"✅ Correct: {mcq[8]}\n📘 {mcq[9]}",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("⏮ Prev", callback_data="wrong_prev"),
...
def generate_pdf(uid, exam, topic, attempts, score, total):
    ...
    def watermark(c, d):
        ...
        c.drawString(40, 20, BOT_NAME)   # 👈 corner branding
