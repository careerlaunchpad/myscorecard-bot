...
async def topic_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if total_q == 0:
        await q.edit_message_text("❌ No questions available.", reply_markup=home_keyboard())
        return
...
async def wrong_only(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.user_data.get("wrong"):
        await q.edit_message_text("🎉 No wrong questions!", reply_markup=home_keyboard())
        return
...
async def show_wrong(q, context):
    ...
    if idx > 0:
        buttons.append(InlineKeyboardButton("⬅ Prev", callback_data="wrong_prev"))
    if idx < len(wrong) - 1:
        buttons.append(InlineKeyboardButton("Next ➡", callback_data="wrong_next"))
...
async def myscore(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send(msg, parse_mode="Markdown", reply_markup=home_keyboard())
...
def generate_pdf(uid, context):
    ...
    story = []
    story.append(Paragraph(f"{BOT_BRAND} – टेस्ट परिणाम", styles["Title"]))
...
async def pdf_result(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_document(
        ...
        filename=f"{BOT_BRAND}_Result.pdf"
    )
